...
import {
  ...
  FaFileAlt, FaGraduationCap, FaListAlt, FaPaperclip, FaSchool,
  FaCheckCircle, FaHourglassHalf, FaCheck, FaTimes, FaArrowLeft
} from 'react-icons/fa';
...
export default function Educacao() {
  ...
  const [formData, setFormData] = useState({
    escola: '', municipio: 'Cazenga', bairro: 'Hoji-ya-Henda', nomeRua: '',
    localEspecificoDaOcorrencia: '', dataOcorrecia: '', subtipo: '',
    descricaoDetalhada: '', nome: '', contacto: '', email: '', anexo: null
  });
  ...
  const tiposProblema = [
    { value: 'professor_faltou', label: 'Falta de Professores', icon: '👨‍🏫' },
    { value: 'infraestrutura_ruim', label: 'Infraestrutura precária', icon: '🏚️' },
    { value: 'livros_faltando', label: 'Falta de material didático', icon: '📚' },
    { value: 'falta_merenda', label: 'Falta de merenda escolar', icon: '🍎' },
    { value: 'corrupcao', label: 'Corrupção ou Pagamento Indevido', icon: '💰' },
    { value: 'fissuras', label: 'Infraestrutura Escolar Degradada', icon: '🏛️' },
    { value: 'assedio', label: 'Assédio', icon: '⚠️' },
    { value: 'sem_carteiras', label: 'Falta de carteiras', icon: '🪑' },
    { value: 'banheiros', label: 'Casas de Banho Degradadas', icon: '🚽' }
  ];

  const validarCampo = (name, value) => {
    const v = value?.toString().trim() || '';
    switch (name) {
      case 'escola': return !v ? 'Nome da escola é obrigatório' : (v.length < 3 ? 'Mínimo 3 caracteres' : null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({
          escola: '', municipio: 'Cazenga', bairro: 'Hoji-ya-Henda', nomeRua: '',
          localEspecificoDaOcorrencia: '',  anexo: '', dataOcorrecia: '', subtipo: '',
          descricaoDetalhada: '', nome: '', contacto: '', email:  null
        });
 ...
 const gerarImagem = async () => {
  ...
  useEffect(() => {
    fetch("http://localhost:9090/api/denuncias")
      .then(res => res.json())
      .then(data => setDenuncias(data.filter(d => d.categoriaNome === "Educação") || []))
      .catch(console.error);
  }, []);
      ...
      <div style={{ minHeight: '100vh', background: '#f5f5f5', fontFamily: "'Inter', sans-serif" }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%)', padding: '3rem 1rem', textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', padding: '1rem', borderRadius: '50%', background: 'rgba(212,175,55,0.15)', marginBottom: '1rem' }}>
            <FaGraduationCap style={{ fontSize: '2rem', color: '#D4AF37' }} />
          </div>
          <h1 style={{ color: '#D4AF37', fontSize: '2rem', fontWeight: 'bold', margin: 0 }}>Registrar Denúncia - Educação</h1>
        ...
        <div style={{ maxWidth: '800px', margin: '-40px auto 0', padding: '2rem', background: 'white', borderRadius: '20px', boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <label style={{ fontWeight: '600', marginBottom: '0.5rem', display: 'block' }}><FaSchool style={{ color: '#D4AF37' }} /> Nome da Escola *</label>
            <input type="text" name="escola" value={formData.escola} onChange={handleChange} onBlur={handleBlur} placeholder="Digite o nome completo da escola" style={{ width: '100%', padding: '0.75rem', borderRadius: '12px', border: `2px solid ${errors.escola ? '#ef4444' : '#e5e7eb'}` }} />
            {errors.escola && <small style={{ color: '#ef4444' }}>{errors.escola}</small>}
          </div>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <label style={{ fontWeight: '600', marginBottom: '0.5rem', display: 'block' }}><FaMapMarkerAlt style={{ color: '#D4AF37' }} /> Local Específico *</label>
            <input type="text" name="localEspecificoDaOcorrencia" value={formData.localEspecificoDaOcorrencia} onChange={handleChange} onBlur={handleBlur} placeholder="Ex: Sala de aula, pátio, diretoria..." style={{ width: '100%', padding: '0.75rem', borderRadius: '12px', border: `2px solid ${errors.localEspecificoDaOcorrencia ? '#ef4444' : '#e5e7eb'}` }} />
        ...
        {denunciaFinal && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
            <div style={{ background: 'white', borderRadius: '20px', maxWidth: '600px', width: '90%', padding: '2rem' }}>
              ...
              <hr />
              <p><strong>Escola:</strong> {denunciaFinal.escola}</p>
        ...
        <div ref={comprovativoRef} style={{ position: 'fixed', top: '-9999px', left: 0,  width: '800px', padding: '20px', background: 'white' }}>
          {denunciaFinal && (
            <div style={{ padding: '20px', border: '1px solid #D4AF37', borderRadius: '10px' }}>
              ...
              <h4 style={{ textAlign: 'center' }}>COMPROVATIVO DE DENÚNCIA - EDUCAÇÃO</h4>
              <hr />
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem 1rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        <FaGraduationCap style={{ color: '#D4AF37', fontSize: '2rem' }} />
        <h2 style={{ color: '#D4AF37', margin: 0, fontWeight: 'bold' }}>Denúncias - Educação</h2>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
            <thead>
              <tr style={{ background: '#1a1a1a', color: '#D4AF37' }}>
                <th style={{ padding: '1rem', textAlign: 'left' }}>Escola</th>
            ...
            <tbody>
              {denuncias.map(d => (
                <tr key={d.pkDenuncia} style={{ borderBottom: '1px solid #e5e7eb' }}>
                  <td style={{ padding: '1rem' }}><strong>{d.escola}</strong></td>
