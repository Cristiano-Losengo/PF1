...
export default function ContaListar() {
  ...
  useEffect(() => {
    console.log("🚀 ContaListar CARREGANDO...");
  }, []);
  ...
  const carregarDados = async () => {
    try {
      ...
      // ADICIONADO: Verificação das respostas
      console.log("📥 Resposta da API de contas:", {
      ...
      if (contasRes.ok) {
        ...
        console.log("✅ Dados de contas carregados:", contasData.length, "registros");
      } else {
        console.error("❌ Erro ao carregar contas:", contasRes.status, contasRes.statusText);
      ...
      console.log("🗺️ Perfis mapeados:", Object.keys(perfisMap).length);
      ...
      const contasCompletas = contasData.map(conta => {
        ...
        console.log("🏷️ fkPerfil direto da conta:", fkPerfilDaConta);
      ...
      if (error.message.includes("Failed to fetch")) {
        console.log("⚠️ API não disponível. Verifique se o servidor está rodando na porta 9090.");
        setMensagem({
          tipo: "warning",
          texto: "Servidor API não disponível. Verifique se o servidor está rodando."
        });
      }
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    console.log("🔄 useEffect do ContaListar rodando");
    carregarDados();
  }, []);
  ...
  // CORRIGIDO: Função para editar conta usando apenas ID na URL
  const handleEditarConta = (conta) => {
    console.log("✏️ INICIANDO EDIÇÃO DA CONTA");
    console.log("📝 ID da conta:", conta.pkConta);
    console.log("👤 Nome da conta:", conta.nomeCompleto);
    
    // Usar apenas o ID na URL - método mais confiável
    ...
    return (
      <div className="container mt-5 d-flex justify-content-center">
        <div className="alert alert-info text-center w-100" style={{ maxWidth: '800px' }}>
          ...
          <div className="mt-2">
            <small className="text-muted">Aguarde enquanto buscamos as informações</small>
          </div>
        </div>
      </div>
    );
      ...
      {mensagem && (
        <div className={`alert alert-${mensagem.tipo} alert-dismissible fade show`} role="alert">
          <div className="d-flex align-items-center">
            ...
            {mensagem.tipo === "warning" && <span className="me-2">⚠️</span>}
              ...
              <div className="modal-footer">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    // Usar a mesma função handleEditarConta
